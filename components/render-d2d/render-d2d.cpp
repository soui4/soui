...
class D2DColor {
  public:
    D2DColor(BYTE r, BYTE g, BYTE b, BYTE a = 0xFF)
        : a(a)
        , b(b)
        , g(g)
        , r(r)
...
BOOL SRenderFactory_D2D::Init()
{
    ...
    hr = DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, IID_IDWriteFactory, reinterpret_cast<IUnknown **>(&m_pDWriteFactory));
    if (!SUCCEEDED(hr))
        return FALSE;

    // Create WIC factory.
    hr = CoCreateInstance(CLSID_WICImagingFactory, NULL, CLSCTX_INPROC_SERVER, IID_IWICImagingFactory, reinterpret_cast<void **>(&m_pWICImageFactory));
    if (!SUCCEEDED(hr))
        return FALSE;
...
SPen_D2D::SPen_D2D(IRenderFactory *pRenderFac, int iStyle /*=PS_SOLID*/, COLORREF cr /*=0*/, int cWidth /*=1*/)
    : TD2DRenderObjImpl<IPenS, OT_PEN>(pRenderFac)
    , m_nWidth(cWidth)
    , m_style(iStyle)
    , m_cr(cr)
{
...
void SPen_D2D::UpdatePen()
{
    ...
    D2D1_STROKE_STYLE_PROPERTIES prop = D2D1::StrokeStyleProperties(D2D1_CAP_STYLE_SQUARE, D2D1_CAP_STYLE_SQUARE, D2D1_CAP_STYLE_FLAT, D2D1_LINE_JOIN_ROUND, 0.0f, D2D1_DASH_STYLE_SOLID, 0.0f);
...
BOOL SFont_D2D::CreateD2DFont()
{
    ...
    HRESULT hr = pFac->CreateTextFormat(strFaceName.c_str(), NULL, ConvertFontWeight(m_lf.lfWeight), m_lf.lfItalic ? DWRITE_FONT_STYLE_ITALIC : DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, abs(m_lf.lfHeight), L"", &m_hFont);
    SASSERT(SUCCEEDED(hr));
    return hr == S_OK;
}

//////////////////////////////////////////////////////////////////////////
SBrush_D2D::SBrush_D2D(IRenderFactory *pRenderFac, COLORREF cr)
    : TD2DRenderObjImpl<IBrushS, OT_BRUSH>(pRenderFac)
    , m_cr(cr)
    , m_brushType(Brush_Color)
{
}

SBrush_D2D::SBrush_D2D(IRenderFactory *pRenderFac, IWICBitmap *hBmp)
    : TD2DRenderObjImpl<IBrushS, OT_BRUSH>(pRenderFac)
    , m_cr(CR_INVALID)
...
SBrush_D2D::SBrush_D2D(IRenderFactory *pRenderFac, const GradientItem *pGradients, int nCount, const GradientInfo *info, BYTE byAlpha, TileMode tileMode)
    : TD2DRenderObjImpl<IBrushS, OT_BRUSH>(pRenderFac)
...
void SRegion_D2D::Offset(POINT pt)
{
    ...
    HRESULT hr = pFac->CreateTransformedGeometry(m_hRgn, D2D1::Matrix3x2F::Translation(pt.x, pt.y), &hrgn2);
    m_hRgn = hrgn2;
}
    ...
    , m_xferMode(kSrcOver_Mode)    // 默认混合模式
{
    ...
    HRESULT hr = pD2DFac->CreateHwndRenderTarget(D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_HARDWARE, pixelFormat, 0.0, 0.0, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE), D2D1::HwndRenderTargetProperties(m_hWnd, size, D2D1_PRESENT_OPTIONS_RETAIN_CONTENTS), &rt);
    if (SUCCEEDED(hr))
    {
        rt->SetDpi(96, 96);
        m_rt = rt;
...
HRESULT SRenderTarget_D2D::BitBlt(LPCRECT pRcDest, IRenderTarget *pRTSour, int xSrc, int ySrc, DWORD dwRop /*=SRCCOPY*/)
{
    ...
    ApplyXfermodeForBitmap(pSrc->toD2D1Bitmap(m_rt), toRectF(pRcDest), 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR, &rcSrcF);
    if (bDrawing)
...
HRESULT SRenderTarget_D2D::DrawText(LPCTSTR pszText, int cchLen, LPRECT pRc, UINT uFormat)
{
    ...
    if (uFormat & DT_SINGLELINE)
        txtFormat->SetWordWrapping(DWRITE_WORD_WRAPPING_NO_WRAP);
    else
        txtFormat->SetWordWrapping(DWRITE_WORD_WRAPPING_WRAP);

    if (uFormat & DT_CENTER)
        txtFormat->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER);
    else if (uFormat & DT_RIGHT)
        txtFormat->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_TRAILING);
    else
        txtFormat->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_LEADING);

    if (uFormat & DT_VCENTER)
        txtFormat->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
    else if (uFormat & DT_BOTTOM)
        txtFormat->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_FAR);
    else
        txtFormat->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_NEAR);
...
HRESULT SRenderTarget_D2D::DrawIconEx(int xLeft, int yTop, HICON hIcon, int cxWidth, int cyWidth, UINT diFlags)
{
    ...
    HRESULT hr = imgFac->CreateBitmapFromHBITMAP(iconInfo.hbmColor, NULL, WICBitmapUsePremultipliedAlpha, &bmp);
    if (hr == S_OK)
    {
        ...
        if (hr == S_OK)
        {
            ...
            ApplyXfermodeForBitmap(pBitmap, rcDest, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, NULL);
        }
    }
...
HRESULT SRenderTarget_D2D::DrawBitmap(LPCRECT pRcDest, const IBitmapS *pBitmap, int xSrc, int ySrc, BYTE byAlpha /*=0xFF*/)
{
    ...
    ApplyXfermodeForBitmap(bmp, toRectF(pRcDest), float(byAlpha) / 255.f, D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR, &rcSrc);
    return S_OK;
}
...
HRESULT SRenderTarget_D2D::AlphaBlend(LPCRECT pRcDest, IRenderTarget *pRTSrc, LPCRECT pRcSrc, BYTE byAlpha)
{
    ...
    rcSrc.OffsetRect(pRTGdiSrc->m_ptOrg);
    D2D1_RECT_F rcSrcF = toRectF(&rcSrc);
    ApplyXfermodeForBitmap(pSrc->toD2D1Bitmap(m_rt), toRectF(pRcDest), float(byAlpha) / 255.f, D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR, &rcSrcF);
    if (bDrawing)
        pRTGdiSrc->m_rt->BeginDraw();
...
HRESULT SRenderTarget_D2D::DrawBitmap9Patch(LPCRECT pRcDest, const IBitmapS *pBitmap, LPCRECT pRcSrc, LPCRECT pRcSourMargin, UINT expendMode, BYTE byAlpha /*=0xFF*/)
{
    ...
    // 定义绘制模式
    UINT mode[3][3] = { { EM_NULL, expendMode, EM_NULL }, { expendMode, expendMode, expendMode }, { EM_NULL, expendMode, EM_NULL } };
...
HRESULT SRenderTarget_D2D::DrawGradientRect(LPCRECT pRect, BOOL bVert, POINT ptRoundCorner, const GradientItem *pGradients, int nCount, BYTE byAlpha)
{
    GradientInfo info;
    info.type = linear;
...
HRESULT SRenderTarget_D2D::DrawGradientRectEx(LPCRECT pRect, POINT ptRoundCorner, const GradientItem *pGradients, int nCount, const GradientInfo *info, BYTE byAlpha)
{
    ID2D1Factory *fac = toD2DFac(m_pRenderFactory);
...
HRESULT SRenderTarget_D2D::SetTransform(const float matrix[9], float oldMatrix[9])
{
    ...
    Matrix3x2F xForm(matrix[kMScaleX], matrix[kMSkewY], matrix[kMSkewX], matrix[kMScaleY], matrix[kMTransX], matrix[kMTransY]);
    if (oldMatrix)
    {
...
// 辅助函数：应用混合模式到位图绘制
void SRenderTarget_D2D::ApplyXfermodeForBitmap(ID2D1Bitmap *pBitmap, const D2D1_RECT_F &rcDest, float opacity, D2D1_BITMAP_INTERPOLATION_MODE interpolationMode, const D2D1_RECT_F *pRcSrc)
{
    // 对于默认的SrcOver模式，直接绘制
    if (m_xferMode == kSrcOver_Mode)
    {
    ...
    if (SUCCEEDED(hr) && deviceContext)
    {
        ...
        if (MapXfermodeToCompositeMode(m_xferMode, &compositeMode))
        {
            ...
            if (needScale)
            {
                ...
                deviceContext->DrawImage(pBitmap, &destPoint, &srcRect, interpolationMode == D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR ? D2D1_INTERPOLATION_MODE_NEAREST_NEIGHBOR : D2D1_INTERPOLATION_MODE_LINEAR, compositeMode);
    ...
    // 对于基本ROP2模式，使用GDI互操作
    if (m_xferMode == kSrcCopy || m_xferMode == kDstInvert || m_xferMode == kSrcInvert || m_xferMode == kSrcAnd)
    {
        // 使用GDI互操作实现ROP2模式
        SComPtr<ID2D1GdiInteropRenderTarget> gdiRt;
        ...
        if (SUCCEEDED(hr) && gdiRt)
        {
            ...
            if (SUCCEEDED(hr) && hdc)
            {
                ...
                RECT rcUpdate = { (LONG)rcDest.left, (LONG)rcDest.top, (LONG)rcDest.right, (LONG)rcDest.bottom };
                gdiRt->ReleaseDC(&rcUpdate);
                return;
            }
        }
...
// 辅助函数：创建临时位图用于几何图形渲染
static HRESULT CreateTemporaryBitmap(ID2D1RenderTarget *pRT, const D2D1_RECT_F &bounds, ID2D1Bitmap **ppBitmap, ID2D1BitmapRenderTarget **ppBitmapRT)
{
    ...
    HRESULT hr = pRT->CreateCompatibleRenderTarget(&size, NULL, NULL, D2D1_COMPATIBLE_RENDER_TARGET_OPTIONS_NONE, ppBitmapRT);
        ...
        if (MapXfermodeToCompositeMode(m_xferMode, &compositeMode))
        {
            ...
            if (SUCCEEDED(hr) && tempBitmap && bitmapRT)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    deviceContext->DrawImage(tempBitmap, &destPoint, &srcRect, D2D1_INTERPOLATION_MODE_LINEAR, compositeMode);
                    return;
                }
            }
            // 如果创建临时位图失败，回退到基本实现
...
static D2D1_COLOR_F nsvgPaintToD2DColor(const NSVGpaint *paint, float opacity)
{
    if (paint && paint->type == NSVG_PAINT_COLOR)
        return nsvgColorToD2DColor(paint->color, opacity);
    return nsvgColorToD2DColor(0xFF000000, opacity);
}
...
static void nsvgPathToD2DGeometry(const NSVGpath *nsvgPath, ID2D1Factory *pFactory, D2D1_FILL_MODE fillMode, ID2D1PathGeometry **ppGeometry)
{
    *ppGeometry = NULL;
...
static float nsvgGetTextBaseline(IDWriteTextLayout *pTextLayout, const DWRITE_TEXT_METRICS &metrics)
{
    ...
    if (FAILED(pTextLayout->GetLineMetrics(&lineMetrics, 1, &actualLineCount)) || actualLineCount == 0 || lineMetrics.baseline <= 0.0f)
        return metrics.height;

    return lineMetrics.baseline;
}

static bool nsvgGetTextHitTestBounds(IDWriteTextLayout *pTextLayout, UINT32 textLength, const DWRITE_TEXT_METRICS &metrics, float originX, float originY, D2D1_RECT_F *pBounds)
{
    if (!pTextLayout || !pBounds || textLength == 0)
        return false;

    UINT32 maxHitTestMetricsCount = metrics.lineCount * metrics.maxBidiReorderingDepth;
    if (maxHitTestMetricsCount == 0)
        maxHitTestMetricsCount = 1;

    std::vector<DWRITE_HIT_TEST_METRICS> hitTestMetrics(maxHitTestMetricsCount);
    UINT32 actualHitTestMetricsCount = 0;
    HRESULT hr = pTextLayout->HitTestTextRange(0, textLength, originX, originY, &hitTestMetrics[0], maxHitTestMetricsCount, &actualHitTestMetricsCount);

    if (hr == HRESULT_FROM_WIN32(ERROR_INSUFFICIENT_BUFFER) && actualHitTestMetricsCount > maxHitTestMetricsCount)
    {
        hitTestMetrics.resize(actualHitTestMetricsCount);
        maxHitTestMetricsCount = actualHitTestMetricsCount;
        hr = pTextLayout->HitTestTextRange(0, textLength, originX, originY, &hitTestMetrics[0], maxHitTestMetricsCount, &actualHitTestMetricsCount);
    }

    if (FAILED(hr) || actualHitTestMetricsCount == 0)
        return false;

    float minX = hitTestMetrics[0].left;
    float minY = hitTestMetrics[0].top;
    float maxX = hitTestMetrics[0].left + hitTestMetrics[0].width;
    float maxY = hitTestMetrics[0].top + hitTestMetrics[0].height;

    for (UINT32 i = 1; i < actualHitTestMetricsCount; ++i)
    {
        const DWRITE_HIT_TEST_METRICS &hit = hitTestMetrics[i];
        minX = (std::min)(minX, hit.left);
        minY = (std::min)(minY, hit.top);
        maxX = (std::max)(maxX, hit.left + hit.width);
        maxY = (std::max)(maxY, hit.top + hit.height);
    }

    *pBounds = D2D1::RectF(minX, minY, maxX, maxY);
    return true;
}

static bool nsvgTextIntersectsSrcRect(const NSVGtext *text, IDWriteTextLayout *pTextLayout, UINT32 textLength, LPCRECT prcSrc)
{
    if (!text || !prcSrc || !pTextLayout)
        return true;

    DWRITE_TEXT_METRICS metrics = {};
    if (FAILED(pTextLayout->GetMetrics(&metrics)))
        return true;

    float baseline = nsvgGetTextBaseline(pTextLayout, metrics);

    float offsetX = 0.0f;
    switch (text->anchor)
    {
    case NSVG_ANCHOR_MIDDLE:
        offsetX = -metrics.widthIncludingTrailingWhitespace / 2.0f;
        break;
    case NSVG_ANCHOR_END:
        offsetX = -metrics.widthIncludingTrailingWhitespace;
        break;
    default:
        break;
    }
    ...
    float localBottom = 0.0f;

    D2D1_RECT_F hitTestBounds = {};
    if (nsvgGetTextHitTestBounds(pTextLayout, textLength, metrics, offsetX, -baseline, &hitTestBounds))
    {
        localLeft = hitTestBounds.left;
        localTop = hitTestBounds.top;
        localRight = hitTestBounds.right;
        localBottom = hitTestBounds.bottom;
    }
    else
    {
        DWRITE_OVERHANG_METRICS overhang = {};
        pTextLayout->GetOverhangMetrics(&overhang);
        localLeft = offsetX - overhang.left;
        localTop = -baseline - overhang.top;
        localRight = offsetX + metrics.widthIncludingTrailingWhitespace + overhang.right;
    ...
    D2D1_POINT_2F pts[4] = {
        ...
        D2D1::Point2F(localRight, localTop),
        D2D1::Point2F(localRight, localBottom),
        D2D1::Point2F(localLeft, localBottom),
    };

    for (int i = 0; i < 4; ++i)
        pts[i] = textMatrix.TransformPoint(pts[i]);
...
static void renderNSVGshape(ID2D1RenderTarget *pRT, ID2D1Factory *pFactory, const NSVGimage *image, const NSVGshape *shape, BYTE byAlpha, LPCRECT prcSrc)
{
    ...
    for (const NSVGpath *path = shape->paths; path != NULL; path = path->next)
    {
        ...
        nsvgPathToD2DGeometry(path, pFactory, fillMode, &pGeometry);
        if (!pGeometry)
            continue;
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            if (order == NSVG_PAINT_FILL && shape->fill.type != NSVG_PAINT_NONE)
            {
                ...
                else if (shape->fill.type == NSVG_PAINT_LINEAR_GRADIENT || shape->fill.type == NSVG_PAINT_RADIAL_GRADIENT)
                {
                    ...
                    if (grad && grad->nstops > 0)
                    {
                        ...
                        D2D1_MATRIX_3X2_F gradMatrix = D2D1::Matrix3x2F(grad->xform[0], grad->xform[1], grad->xform[2], grad->xform[3], grad->xform[4], grad->xform[5]);
            ...
            else if (order == NSVG_PAINT_STROKE && shape->stroke.type != NSVG_PAINT_NONE && shape->strokeWidth > 0)
            {
                ...
                strokeProps.miterLimit = shape->miterLimit;
                strokeProps.dashStyle = D2D1_DASH_STYLE_SOLID;
                strokeProps.dashOffset = shape->strokeDashOffset;
                ...
                if (shape->strokeDashCount > 0)
                {
                    ...
                    dashCount = shape->strokeDashCount;
                }
...
{
    if (!text || !pRT || !pWriteFactory || text->text[0] == '\0')
        return;
    ...
    DWRITE_FONT_WEIGHT fontWeight = text->fontWeight >= 7 ? DWRITE_FONT_WEIGHT_BOLD : DWRITE_FONT_WEIGHT_NORMAL;
    DWRITE_FONT_STYLE fontStyle = (text->fontStyle == 1 || text->fontStyle == 2) ? DWRITE_FONT_STYLE_ITALIC : DWRITE_FONT_STYLE_NORMAL;

    SComPtr<IDWriteTextFormat> pTextFormat;
    HRESULT hr = pWriteFactory->CreateTextFormat(S_CA2W(text->fontFamily), NULL, fontWeight, fontStyle, DWRITE_FONT_STRETCH_NORMAL, fontSize, L"", &pTextFormat);
    ...
    hr = pWriteFactory->CreateTextLayout(strText, strText.GetLength(), pTextFormat, 10000.0f, 10000.0f, &pTextLayout);
    if (FAILED(hr) || !pTextLayout)
        return;

    UINT32 textLength = (UINT32)strText.GetLength();
    if (!nsvgTextIntersectsSrcRect(text, pTextLayout, textLength, prcSrc))
        return;
    ...
    pRT->CreateSolidColorBrush(nsvgColorToD2DColor(text->fillColor, opacity), &pTextBrush);
    if (!pTextBrush)
        return;
    ...
    {
        float localX = 0.0f;
        float localY = -baseline;
        switch (text->anchor)
        {
        case NSVG_ANCHOR_MIDDLE:
            localX -= metrics.widthIncludingTrailingWhitespace / 2.0f;
            break;
        case NSVG_ANCHOR_END:
            localX -= metrics.widthIncludingTrailingWhitespace;
            break;
        default:
            break;
        }
        ...
        pRT->SetTransform(textMatrix * oldTransform);
        pRT->DrawTextLayout(D2D1::Point2F(localX, localY), pTextLayout, pTextBrush);
        pRT->SetTransform(oldTransform);
    }
    else
    {
        pRT->DrawTextLayout(D2D1::Point2F(x, y - baseline), pTextLayout, pTextBrush);
    }
}

HRESULT SRenderTarget_D2D::DrawSVG(ISvgObj *pSvgObj, LPCRECT pRect, LPCRECT prcSrc, BYTE byAlpha)
{
    if (!pSvgObj || !pRect)
        return E_INVALIDARG;
    if (!m_rt)
...
void SPath_D2D::addRect2(THIS_ float left, float top, float right, float bottom, Direction dir /* = kCW_Direction*/)
{
    if (!m_sink)
        return;
    D2D1_POINT_2F points[4] = { { right, top }, { right, bottom }, { left, bottom }, { left, top } };
...
void SPath_D2D::addString(THIS_ LPCTSTR pszText, int nLen, float x, float y, const IFontS *pFont)
{
    ...
    SComPtr<IDWriteTextLayout> pTextLayout;
    HRESULT hr = pDWriteFac->CreateTextLayout(pszText, nLen, pTextFormat, 10000.0f, 10000.0f, &pTextLayout);
    if (FAILED(hr) || !pTextLayout)
        return;
    ...
    {
        SComPtr<ID2D1TransformedGeometry> tg;
        if (!SUCCEEDED(pD2DFac->CreateTransformedGeometry(pTextPath, Matrix3x2F::Translation(x, y), &tg)))
            return;
        pTextPath = tg;
    }
    ...
    else
    {
        ...
        hr = m_geometry->CombineWithGeometry(pTextPath, D2D1_COMBINE_MODE_UNION, NULL, D2D1_DEFAULT_FLATTENING_TOLERANCE, pCombinedSink);
