...
public:
    ...
    STDMETHOD_(BOOL, CreateMaskFilter)(THIS_ REFGUID guidEffect, IMaskFilter * *ppMaskFilter) OVERRIDE {return FALSE;}
    STDMETHOD_(BOOL, CreateImageFilter)(THIS_ REFGUID guidEffect, IImageFilter * *ppImageFilter) OVERRIDE {return FALSE;}
...
public:
	...
	SBrush_GDI(IRenderFactory * pRenderFac,HBITMAP hBmp, TileMode tileMode);
	SBrush_GDI(IRenderFactory* pRenderFac, const GradientItem* pGradients, int nCount, const GradientInfo* info, BYTE byAlpha, TileMode tileMode);
	~SBrush_GDI();
...
protected:
	...
	SIZE        m_sz;
	mutable HBITMAP     m_hBmp;     //标准的32位位图，和m_bitmap共享内存
};
...
public:
	...
	STDMETHOD_(HRESULT,GetFontMetrics)(THIS_ TEXTMETRIC *ptm) OVERRIDE;
	...
	STDMETHOD_(HRESULT, DrawGradientPath)
		(THIS_ const IPathS* path, const GradientItem *pGradients, int nCount,const GradientInfo *info, BYTE byAlpha DEF_VAL(0xFF), LPCRECT pRcBox DEF_VALUE(NULL)) OVERRIDE;
