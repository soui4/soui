...
HANDLE WINAPI LoadImageBuf(const void* bits, UINT filesize, UINT type,
	...
	if(type == IMAGE_BITMAP)
		return 0;
	BOOL fCursor = type == IMAGE_CURSOR?TRUE:FALSE;
	/* Check for .ani. */
	...
	return (HANDLE)create_icon_from_bmi( (const BITMAPINFO *)&((LPBYTE)bits)[entry->dwDIBOffset], filesize - entry->dwDIBOffset,
