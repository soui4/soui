...
macro(config_compiler_and_linker)
    ...
    if (MSVC)
      if(NOT SOUI_SHARED_CRT)
        ...
        add_compile_options($<$<CONFIG:RelWithDebInfo>:/MT>)   # Runtime Library: /MT  = MultiThreaded Runtime
        set(BUILD_CONF_MT "1")
      else()
        ...
        add_compile_options($<$<CONFIG:RelWithDebInfo>:/MD>)   # Runtime Library: /MT  = MultiThreaded Runtime
        set(BUILD_CONF_MT "0")
     endif(NOT SOUI_SHARED_CRT)
    endif(MSVC)
    if (NOT SOUI_WCHAR_AS_DEFAULT)
    ...
    if (SOUI_XP_TOOLSET) 
        ...
        set(BUILD_CONF_SOUI_XP_TOOLSET "1")
    else(SOUI_XP_TOOLSET)
        set(BUILD_CONF_SOUI_XP_TOOLSET "0")
    endif()

    #
    # 启用Spectre缓解
    #
    if (SOUI_ENABLE_SPECTRE)
    ...
    if (SOUI_ENABLE_CORE_LIB) 
        set(BUILD_CONF_SOUI_ENABLE_CORE_LIB "1")
    else(SOUI_XP_TOOLSET)
        set(BUILD_CONF_SOUI_ENABLE_CORE_LIB "0")
    endif()
    if (SOUI_ENABLE_COM_LIB) 
        set(BUILD_CONF_SOUI_ENABLE_COM_LIB "1")
    else(SOUI_XP_TOOLSET)
        set(BUILD_CONF_SOUI_ENABLE_COM_LIB "0")
    endif()
endmacro()

function(detectArmTarget IsArm)
set(IS_ARM 0)
if(APPLE)
    message(STATUS "CMAKE_OSX_ARCHITECTURES: ${CMAKE_OSX_ARCHITECTURES}, CMAKE_SYSTEM_PROCESSOR = ${CMAKE_SYSTEM_PROCESSOR}")
    if(CMAKE_OSX_ARCHITECTURES MATCHES "x86_64|x86")
        set(IS_ARM 0)
    elseif(CMAKE_OSX_ARCHITECTURES MATCHES "arm|arm64")
        set(IS_ARM 1)
    ...
    message(STATUS "CMAKE_GENERATOR_PLATFORM: ${CMAKE_GENERATOR_PLATFORM}, CMAKE_SYSTEM_PROCESSOR: ${CMAKE_SYSTEM_PROCESSOR}")
    if(CMAKE_GENERATOR_PLATFORM MATCHES "ARM|ARM64")
        set(IS_ARM 1)
    elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "arm|arm64|aarch64")
        set(IS_ARM 1)
    else()
