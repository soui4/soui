...
static HRESULT CreateCoder2(bool encode, UInt32 index, const GUID *iid, void **outObject)
{
  *outObject = NULL;
  ...
  if (codec.IsFilter)
  {
    if (!IsEqualGUID(*iid,IID_ICompressFilter)) return E_NOINTERFACE;
  }
  else if (codec.NumStreams != 1)
  {
    if (!IsEqualGUID(*iid,IID_ICompressCoder2)) return E_NOINTERFACE;
  }
  else
  {
    if (!IsEqualGUID(*iid,IID_ICompressCoder)) return E_NOINTERFACE;
  }
