...
// NOTE: the SK_RESTRICT qualifiers below are not cosmetic.  The definitions of these
// functions are generated by SkBitmapProcState_sample.h / SkBitmapProcState_shaderproc.h,
// which declare every pointer parameter as `T* SK_RESTRICT`.  The MSVC ABI *does* encode
// __restrict in the mangled name (verified for both cl.exe and clang-cl: `T*` -> PEAG,
// `T* __restrict` -> PEIAG), so a declaration without SK_RESTRICT produces a different
// symbol than the definition and the link fails.  The Itanium ABI (GCC/Clang on
// Linux/Android/OHOS) ignores restrict when mangling, which is why this mismatch stayed
// latent until the NEON path was compiled for a Windows-on-ARM64 target.
extern void  S16_D16_filter_DX_neon(const SkBitmapProcState&, const uint32_t* SK_RESTRICT, int, uint16_t* SK_RESTRICT);
extern void  Clamp_S16_D16_filter_DX_shaderproc_neon(const SkBitmapProcState&, int, int, uint16_t* SK_RESTRICT, int);
extern void  Repeat_S16_D16_filter_DX_shaderproc_neon(const SkBitmapProcState&, int, int, uint16_t* SK_RESTRICT, int);
extern void  SI8_opaque_D32_filter_DX_neon(const SkBitmapProcState&, const uint32_t* SK_RESTRICT, int, SkPMColor* SK_RESTRICT);
extern void  SI8_opaque_D32_filter_DX_shaderproc_neon(const SkBitmapProcState&, int, int, uint32_t* SK_RESTRICT, int);
extern void  Clamp_SI8_opaque_D32_filter_DX_shaderproc_neon(const SkBitmapProcState&, int, int, uint32_t* SK_RESTRICT, int);
#endif
