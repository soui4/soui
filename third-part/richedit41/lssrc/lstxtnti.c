...
static LSERR CheckReallocGlyphs(PLNOBJ plnobj, LONG cglyphs)
{
	...
	if ( igindLocalStart + cglyphs <= (LONG)pilsobj->gindMax - 2)
		...
		lserr = Realloc(pilsobj, (void**)&pilsobj->pdurGind, sizeof(LONG) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&pilsobj->pginf, sizeof(TXTGINF) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&pilsobj->pduGright, sizeof(LONG) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&pilsobj->plsexpinf, sizeof(LSEXPINFO) * cNew);
		if (lserr != lserrNone) return lserr;

		lserr = Realloc(pilsobj, (void**)&plnobj->pgind, sizeof(GINDEX) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&plnobj->pdupGind, sizeof(LONG) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&plnobj->pgoffs, sizeof(GOFFSET) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&plnobj->pexpt, sizeof(EXPTYPE) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&plnobj->pdupBeforeJust, sizeof(LONG) * cNew);
		if (lserr != lserrNone) return lserr;
		lserr = Realloc(pilsobj, (void**)&plnobj->pgprop, sizeof(GPROP) * cNew);
		if (lserr != lserrNone) return lserr;
