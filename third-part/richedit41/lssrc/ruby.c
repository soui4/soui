    ...
    FMTRES *pfmtres)			/* (OUT): formatting result */
{
	...
	if (RubyPronunciationLineFirst == pilsobj->rubysyntax)
		...
		lserr = FormatLine(pilsobj->plsc, cpStartRuby, INT_MAX, lstflow,
		...
		if (lserrNone == lserr)
			{
			lserr = FormatLine(pilsobj->plsc, cpStartMain, INT_MAX, lstflow,
	...
	*pcActualBreakRecord = 0;
		...
		brkkind != brkkindImposedAfter)
	{
		...
		lserr = FormatLine(pilsobj->plsc, pdobj->cpStart + dcpOffset + 1, 
			INT_MAX, pdobj->lstflow, &pdobj->plssublMain, RUBY_MAIN_ESC_CNT,
