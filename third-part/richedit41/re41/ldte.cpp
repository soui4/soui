	...
	LONG	   lStreamFormat)	//@parm Stream format
{
	...
	fSingleObject = chrg.cpMax - chrg.cpMin == 1 &&
		_ped->HasObjects() &&
		_ped->_pobjmgr->CountObjectsInRange(chrg.cpMin, chrg.cpMax);
	if(precall)
	{
...
void CLightDTEngine::FlushClipboard()
{
	...
	if( _pdo )
	{
		HRESULT hr = OleIsCurrentClipboard(_pdo);
		if(hr == NOERROR )
		{
	...
	IUndoBuilder *publdr)		// @parm undo builder to receive antievents
{
	...
	if(precall)
	{
		...
		// give the callback a chance to give us its own IDataObject
		psel->GetRange(chrg.cpMin, chrg.cpMax);
	...
	cch = psel->GetRange(cpMin, cpMax);	// NB: prg is the selection
	cp1 = psel->GetCp();					// Save active end and signed
	cch1 = psel->GetCch();					//  length for Undo antievent
	CTxtRange rg(_ped, cpMax, cch);		// Use range copy to float over
	...
	_pdt->SetDragInfo(publdr, cpMin, cpMax);
	...
	IUndoBuilder *publdr)		// @parm Undo builder to receive antievents
{
	...
	{										//  TEXT case (see SF_x values)
		...
		if(publdr && !tp.IsAfterEOP())
		{
			...
			if (prg->_rpPF.IsValid())
			{
				...
				pae = gAEDispenser.CreateReplaceFormattingAE(_ped, cpMin, rpPF, 
							cpMax - cpMin, GetParaFormatCache(), ParaFormat);
				if(pae)
			...
			if (prg->_rpCF.IsValid())
			{
				...
				pae = gAEDispenser.CreateReplaceFormattingAE(_ped, cpMin, rpCF, 
							cpMax - cpMin, GetCharFormatCache(), CharFormat);
				if(pae)
					publdr->AddAntiEvent(pae);
			}
		} 
		...
		cpMax = prg->GetCp();
		Assert(pes->dwError != 0 || cpMax >= cpMin);
		...
		if(publdr && cpMax > cpMin)
		{
			...
			pae = gAEDispenser.CreateReplaceRangeAE(_ped, cpMin, cpMax, 0, 
						NULL, NULL, NULL);

			HandleSelectionAEInfo(_ped, publdr, -1, -1, cpMax, 0, 
						SELAE_FORCEREPLACE);
			if(pae)
				publdr->AddAntiEvent(pae);
		}
	...
	CTxtRange *prg)				// @parm range to get text from
{
	...
	LONG	cpMin, cpMax;
	LONG	cch = prg->GetRange(cpMin, cpMax);
	...
	if(cch)
	{
		CRchTxtPtr rtp(*prg);
		rtp.SetCp(cpMin);
		cch = rtp.GetPlainText(cchT, pText, cpMax, FALSE, TRUE);
		AssertSz(cch <= cchT,
	...
	*(pText + cch) = 0;
	...
	HGLOBAL	hTemp = GlobalReAlloc(hText, sizeof(WCHAR)*(cch + 1), GMEM_MOVEABLE);
	...
	LONG		lStreamFormat)	// @parm Stream format
{
	...
	cpMax = min(cpMax, cchText);					// Don't write final CR
	rtp.SetCp(cpMin);
	while(rtp.GetCp() < cpMax)
	{
		...
		cch	= rtp.GetPlainText(WRITESIZE, wszBuf, cpMax, fTextize, TRUE);
		if(!cch)
			break;									// No more to do
