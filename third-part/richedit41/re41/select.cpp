...
CTxtSelection::Invariant() const
{
	...
	if(IsInOutlineView() && _cch)
	{
		...
		// assert in that case.
		if (GetPed()->GetAdjustedTextLength() != cpMax)
		{
			AssertSz((unsigned)(tp.FindEOP(cpMax - cpMin) != 0) == _fSelHasEOP,
	...
	BOOL fScrollIntoView)		//@parm TRUE if should scroll caret into view
{
	...
	if(_cch && (_nSelExpandLevel || _fSelExpandCell))
	{
		...
		{
			if(_nSelExpandLevel)
				FindRow(&cpMin, &cpMax, _nSelExpandLevel);
			else
				FindCell(&cpMin, &cpMax);
			Set(cpMax, cpMax - cpMin);
			if(!_fSelExpandCell)
				_nSelExpandLevel = 0;
		}
	}
	...
	if(IsInOutlineView() && !ped->IsMouseDown() && _rpPF.IsValid())
	{
		...
		if(_cch && (cpMin || cpMax < cchText))
		{
			...
			LONG cpMostSave = cpMax;		//  we need to Set() below
			...
			rp.Move(cpMax - cpMin);		// Go to cpMax
			Assert(cpMax == rp.CalculateCp());
			if(rp.IsCollapsed())
				cpMax += rp.FindExpandedForward();

			if(fCpMinCollapsed || rp.IsCollapsed() && cpMax < cchText)
			{
				if(rp.IsCollapsed())
				{
					rp.Move(cpMin - cpMax);
			...
			if(cpMin != cpMinSave || cpMax != cpMostSave)
				Set(cpMax, cpMax - cpMin);
		}
	...
	if(_cch && (cpMin || cpMax < cchText))
	{
		...
		else if(rp.IsInHidden() &&			// cpMin OK, how about cpMax?
			cpMax < cchText)
	...
	LONG *pcp) const
{
	...
	{
		LONG cpMin, cpMax, cchLast;
		GetRange(cpMin, cpMax);
		rp.SetCp(cpMin, FALSE);			// Selections can't start at EOL
		cch = rp.GetIch();
		*pcp = cpMin - cch;
		rp.SetCp(cpMax, TRUE);			// Selections can't end at BOL
...
void CTxtSelection::UpdateSelection()
{
	...
	if(GetPed()->HasObjects())
	{
		...
		if(pobjmgr)
		{
			...
			NumObjInSel = pobjmgr->CountObjectsInRange(cpMin, cpMax);
	...
	if (NumObjInOldSel && (abs(_cchSel) == 1) &&
		!(cpMin == cpMinSel && cpMax == cpMostSel))
	{
		if(pobjmgr)
	...
	if(_fShowSelection)
	{
		if(!_cch || !cchSelSave ||				// Old/new selection missing,
			cpMax < min(cpSelSave, cpSelNA) ||	//  or new preceeds old,
...
/*
 * 	CTxtSelection::SetSelection(cpFirst, cpMax)
 *
 *	@mfunc
 *		Set selection between two cp's
 *	
 *	@devnote
 *		<p cpFirst> and <p cpMax> must be greater than 0, but may extend
...
void CTxtSelection::SetSelection (
	...
	LONG cpMax)			//@parm End of selection and active end
{
    ...
    if(ped->HasObjects())
    {
        ...
        if(pobjmgr)
        {
			...
			if (pobjactive)
			{
				if (pobjactive != pobjmgr->GetObjectFromCp(cpMin) || cpMax - cpMin > 1)
	...
	Set(cpMax, cpMax - cpMin);		// Set() validates cpMin, cpMax
	...
	HITTEST		Hit) const	//@parm May be computer Hit value
{
	...
	return cpActual >= cpMin && cpActual < cpMax;
}
	...
	const POINTUV pt)			//@parm Point of click
{
	...
	if(_pdp->CpFromPoint(pt, NULL, this, NULL, FALSE) >= 0)
	{
		...
		{												// Select word at IP
			...
			// cpMax needs to be the active end
			if(_cch < 0)
				FlipRange();
		}
		Update(FALSE);
	...
	BOOL fExtend)	//@parm Extend range iff TRUE
{
	...
	if(fCollapse)								// Collapse at cpMax
	{
		Collapser(tomEnd);
	...
	BOOL fExtend)	//@parm Extend range iff TRUE
{
	...
	else if(!fExtend && _cch)					// Collapse at cpMax
	{
		Collapser(tomEnd);
		_fCaretNotAtBOL = TRUE;					// Selections can't end at BOL
	}
	...
	BOOL   fExtend)		//@parm Extend range iff TRUE
{
	...
	if((cpMin < cpFirstVisible && _cch <= 0) || (cpMax > cpLastVisible && _cch >= 0))
	{
...
void CTxtSelection::CheckChangeKeyboardLayout ()
{
	...
	{												//  check kbd change
		...
		if (!IsFECharRep(iCharRep) &&
			(iCharRep != ANSI_INDEX || !IsFELCID(LOWORD(GetKeyboardLayout(0)))) &&
	...
	LCID		lcid)		//@parm If nonzero, lcid to use for char
{
	...
	if(ch == TAB && GetPF()->InTable() && !(dwFlags & KBD_CTRL))
	{
		...
		do
		{
			...
			FindCell(&cpMin, &cpMax);
			Assert(cpMax > cpMin);
			cpMax--;								// Don't select the CELL mark
			Set(cpMax, cpMax - cpMin);
		}
		while(cpMax == cpMin + 1 && _rpTX.GetPrevChar() == NOTACHAR);
	...
	DWORD	dwFlags)		//@parm Special flags
{
	...
	if(cpMin  > min(_cpSel, _cpSel + _cchSel) ||// If new sel doesn't
	   cpMax < max(_cpSel, _cpSel + _cchSel))	//  contain all of old
    {                                           //  sel, remove old sel
        ShowSelection(FALSE);
        _fShowCaret = TRUE;     
    }
	...
	{
		Tracef(TRCSEVERR, "CRchTxtPtr::ReplaceRange(%ld, %ld, %ld) failed", GetCp(), cpMax - cpMin, cchNew);
		goto err;
	}
...
err:
	...
	Tracef(TRCSEVERR, "CTxtSelection::ReplaceRange(%ld, %ld)", cpMax - cpMin, cchNew);
 ...
 *		end is at cpMax of a nondegenerate selection, use the PF of the
	...
	DWORD		  dwMask2)	//@parm Second mask
{
	...
	{	
		...
		CTxtPtr tpLast(GetPed(), cpMax - 1);
		if(tpLast.GetChar() == ' ')			// Selection ends with a blank:
		{									
			cpMax--;						// Setup end point to end at last
	...
	SELCHANGE *pselchg)		//@parm SELCHANGE structure to use
{
	...
	if(cch)
	{
		...
		{											//  get count in range
			...
			cObjects = pobjmgr->CountObjectsInRange(cpMin, cpMax);
			if(cObjects > 0)
			{
	...
	const POINTUV pt)		//@parm Point of click
{
	...
	if(GetPed()->TxGetAutoWordSel())	
	{
		...
		{
			// Click is after anchor word, so set cpMax appropriately
			iDir = WB_MOVEWORDLEFT;
