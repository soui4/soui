	...
	IUnknown * punk)
{
	...
	int offset = FIELD_OFFSET(CTxtEdit, _qwCharFlags);
	memset((char*)this + offset, 0, sizeof(CTxtEdit) - offset);
	_unk.Init();
	...
	WCHAR *	   pch)		//@parm Where to put the text
{
	...
	LONG	cpMin  = 0;
	LONG	cpMax = tomForward;
	WCHAR *	pchUse = pch;
	CTempWcharBuf twcb;

	if(pgt->flags & GT_SELECTION)			// Get selected text
	{
		cch = GetSel()->GetRange(cpMin, cpMax);
	...
	if(pgt->flags & GT_NOHIDDENTEXT)
	{
		...
		cch = rtp.GetPlainText(cchGet, pchUse, cpMax, FALSE, pgt->flags & GT_USECRLF);
	}
	else
	{
		...
		if(pgt->flags & GT_RAWTEXT)
			...
			cch = tp.GetPlainText(cchGet, pchUse, cpMax, FALSE, pgt->flags & GT_USECRLF);
	}
	pchUse[cch] = L'\0';
	...
	IUndoBuilder *publdr)			//@parm Undobuilder to receive antievents
{
	...
	if(dwKbdFlags & ALT && dwKbdFlags & CTRL)
	{
		...
		if(vkey == 'E')
		{
			LANGID lid = LANGIDFROMLCID(LOWORD(GetKeyboardLayout(0)));
	...
	BOOL		  fSetupDC)
{
	...
	if(pfr)
	{
		...
		cpMax = GetCpFromAcp(pfr->chrg.cpMax);
	}
	...
	if(fSetDCWorked)
	{
        ...
        if(_pdpPrinter->SetTargetDC(pfr->hdcTarget, dxpInchT, dypInchT))
		{
			...
			cpReturn = _pdpPrinter->FormatRange(cpMin, cpMax, prtcon._fDoPrint);
	...
	TEXTRANGE *pstrg)		//@parm pointer to a text range structure
{
	...
	// Remember that if the cpMax field is -1 and the cpMin field
	...
	if (IsBadReadPtr(pstrg, sizeof(TEXTRANGE))	||
		((pstrg->chrg.cpMax < 1 || pstrg->chrg.cpMin < 0 ||
		 ...
		 !(pstrg->chrg.cpMax == -1 && !pstrg->chrg.cpMin)))
	{
	...
	WCHAR *psz)
{
	...
	LONG cpMax = GetSelMost();
	return GetTextRange(cpMin, cpMax - cpMin + 1, psz);
}
 ...
 *
 *	@mfunc
 *		Get the current selection acpMin, acpMax packaged in a CHARRANGE.
 *	
 *	@comm
...
void CTxtEdit::OnExGetSel(
	CHARRANGE *pcrSel)	//@parm Output parm to receive acpMin, acpMax
{
	TRACEBEGIN(TRCSUBSYSEDIT, TRCSCOPEINTERN, "CTxtEdit::OnExGetSel");

	pcrSel->cpMin  = GetAcpFromCp(GetSelMin());
	pcrSel->cpMax = GetAcpFromCp(GetSelMost());
}

/*
 *	CTxtEdit::OnGetSel (pacpMin, pacpMax)
 *
 *	@mfunc
 *		Get the current selection acpMin, acpMax.
 *	
 *	@rdesc
 *		LRESULT = acpMax > 65535L ? -1 : MAKELRESULT(acpMin, acpMax)
 *
 *	@comm
 *		This function outputs API cp's that may differ from the
 *		corresponding internal Unicode cp's.
...
LRESULT CTxtEdit::OnGetSel(
	...
	LONG *pacpMax)		//@parm Output parm to receive acpMax
{
...
/*
 *	CTxtEdit::OnSetSel (acpMin, acpMax)
 *
 *	@mfunc
 ...
 *
 *		cpMin < 0,  cpMax ???		-- Collapse selection to insertion point
 *									   at text end if cpMax < 0 and else at
 *									   selection active end
 *		cpMin >= 0, cpMax < 0		-- select from cpMin to text end with
 *									   active end at text end
 *
 *		cpMin >= 0, cpMax >= 0		-- Treat as cpMin, cpMax with active
 *									   end at cpMax
 *
 *	@comm
 *		This function inputs API cp's that may differ from the
	...
	LONG acpMax)		//@parm Input acpMax
{
	...
	if(acpMin < 0)
		cpMin = cpMax = (acpMax < 0) ? tomForward : psel->GetCp();
	else
	{
		cpMin  = GetCpFromAcp(acpMin);
		cpMax = (acpMax < 0) ? tomForward : GetCpFromAcp(acpMax);
	}
	if(Get10Mode() && cpMax < cpMin)	// In 10 mode, ensure
	{									//  cpMax >= cpMin.  In
		cpMin ^= cpMax;				//  SetSelection, we set active
		cpMax ^= cpMin;				//  end to cpMax, which can be
		cpMin ^= cpMax;				//  smaller than cpMin, in spite
	}									//  of its name.
	psel->SetSelection(cpMin, cpMax);
	return GetAcpFromCp(psel->GetCpMax());
}
	...
	HANDLE hDropFiles)
{
	...
	if(!fnDragFinish || !fnDragQueryFileW || !fnDragQueryPoint)
	{
...
/*
 *	CTxtEdit::TxFindText (flags, cpMin, cpMax, pch, pcpRet)
 *
 *	@mfunc
 *		Find text in direction specified by flags starting at cpMin if
 *		forward search (flags & FR_DOWN nonzero) and cpMax if backward
 *		search.
 *
 *	@rdesc
...
HRESULT CTxtEdit::TxFindText(
	...
	LONG *		pcpMax) //@parm Out parm to receive end of matched string
{
	...
	FINDTEXTEX *pftex)
{
	...
	if(TxFindText(flags,
				  ...
				  GetCpFromAcp(pftex->chrg.cpMax),
				  pftex->lpstrText, &cpMin, &cpMax) != S_OK)
	{
	...
	{													//  back to change
		...
		pftex->chrgText.cpMax = GetAcpFromCp(cpMax);
	}
...
DWORD CTxtEdit::TxGetMaxLength() const					
{
	...
	// Keep this a DWORD in case client uses a cpMax of 0xFFFFFFFF, which is
	...
	BOOL *	pfInLink)	//@parm if non-NULL, indicate if over a link
{
	...
	LONG	  cpMin, cpMax;			// It's a hyperlink
	ENLINK	  enlink;
	...
	rg.Expander(tomLink, TRUE, NULL, &cpMin, &cpMax);
	...
	{
		rg.Expander(tomHidden, TRUE, NULL, &cpMin, &cpMax);
		rg.SetCp(cpMin, FALSE);
		WCHAR ch;

		//Go to end of hyperlink (search for k)
		if (rg.CRchTxtPtr::FindText(cpMax, FR_DOWN, L"K", 1) == -1)
			return FALSE;
		...
		if (rg.CRchTxtPtr::FindText(cpMax, FR_DOWN, &chPrev, 1) == -1)
			return FALSE;
	...
	enlink.chrg.cpMax = GetAcpFromCp(cpMax);
	...
	LPARAM 	lparam)		//@parm lparam of the msg
{
	...
	prg->GetRange(cpMin, cpMax);

	enp.msg = msg;
	enp.wParam = wparam;
	enp.lParam = lparam;
	enp.chrg.cpMin  = GetAcpFromCp(cpMin);
	enp.chrg.cpMax = GetAcpFromCp(cpMax);
	...
	IUndoBuilder *publdr)	//@parm undo builder to receive antievents
{
	...
	rg.Expander(tomParagraph, TRUE, NULL, &cpMin, &cpMax);
	CPFRunPtr rp(rg);
	...
	if(lparam > 0 && cpMax == GetTextLength())
	{									
		Beep();							// Already at end
		return S_FALSE;
	}
	...
	fDeleteCR = rg.GetCpMax() == GetTextLength();
