 ...
 *		GetCpMax(), GetCpMin(), and GetTextLength() all work in zombie mode,
 ...
 */
LONG CTxtRange::GetCpMax() const
{
	TRACEBEGIN(TRCSUBSYSRANG, TRCSCOPEINTERN, "CTxtRange::GetCpMax");
	...
	BOOL*	pfBaseChar)		// Is pwch[0] a cluster start (base char)?
{
	...
	if (PRIMARYLANGID(LOWORD(hkl)) == LANG_VIETNAMESE)
	{
	...
	else if (PRIMARYLANGID(LOWORD(hkl)) == LANG_THAI ||
