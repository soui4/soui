...
void SListCtrlEx::DeleteItem(int iItem)
{
    ...
    if (m_iSelItem == iItem)
        m_iSelItem = -1;
    else if (m_iSelItem > iItem)
        m_iSelItem--;
    if (m_iHoverItem == iItem)
        m_iHoverItem = -1;
    else if (m_iHoverItem > iItem)
        m_iHoverItem--;
...
int SListCtrlEx::InsertItem(int iItem, LPCWSTR pszXml, LPARAM dwData /*=0*/)
{
    ...
    {
        SXmlDoc xmlDoc;
        if (!xmlDoc.load_buffer(pszXml, wcslen(pszXml) * sizeof(wchar_t), xml_parse_default,
                                enc_utf16))
            return -1;
...
void SListCtrlEx::EnsureVisible(int iItem)
{
    ...
    if (iItem < iFirstVisible || iItem > iFirstVisible + nVisibleItems - 1)
    {
        ...
        if (iItem < iFirstVisible)
            nOffset = (iItem - iFirstVisible) * m_nItemHeight;
        else
            nOffset = (iItem - iFirstVisible - nVisibleItems + 1) * m_nItemHeight;
...
BOOL SListCtrlEx::SetItemCount(int nItems, LPCTSTR pszXmlTemplate)
{
    ...
    if (pszXmlTemplate)
    {
        ...
        if (!xmlDoc.load_buffer((LPCSTR)strUtf8, strUtf8.GetLength(), xml_parse_default, enc_utf8))
            return FALSE;
...
void SListCtrlEx::DrawItem(IRenderTarget *pRT, CRect &rcItem, int iItem)
{
    ...
    else if (((iItem == m_iHoverItem || (m_iHoverItem == -1 && iItem == m_iSelItem)))
             && m_bHotTrack)
    {
        if (m_pItemSkin != NULL)
            nBgImg = 1;
...
BOOL SListCtrlEx::CreateChildren(SXmlNode xmlNode)
{
    ...
    m_pHeader->GetEventSet()->subscribeEvent(EventHeaderItemChanging::EventID,
                                             Subscriber(&SListCtrlEx::OnHeaderSizeChanging, this));
    m_pHeader->GetEventSet()->subscribeEvent(EventHeaderItemSwap::EventID,
                                             Subscriber(&SListCtrlEx::OnHeaderSwap, this));
    ...
    if (xmlTempl)
        m_xmlTempl.root().append_copy(xmlTempl);
...
BOOL SListCtrlEx::OnSetCursor(const CPoint &pt)
{
    ...
    if (m_pCapturedFrame)
    {
        ...
        bRet = m_pCapturedFrame->DoFrameEvent(WM_SETCURSOR, 0,
    ...
    else if (m_iHoverItem != -1)
    {
        ...
        bRet = m_arrItems[m_iHoverItem]->DoFrameEvent(
                   WM_SETCURSOR, 0, MAKELPARAM(pt.x - rcItem.left, pt.y - rcItem.top))
            != 0;
    }
    if (!bRet)
    {
    ...
    else
    {
        ...
        if (iHover != m_iHoverItem)
        {
            ...
            if (m_iHoverItem != -1)
            {
                ...
                m_arrItems[m_iHoverItem]->DoFrameEvent(WM_MOUSEHOVER, wParam,
...
int SListCtrlEx::InsertColumn(int nIndex,
                              LPCTSTR pszText,
                              int nWidth,
                              UINT fmt,
                              LPARAM lParam /*=0*/)
...
BOOL SListCtrlEx::SortItems(PFNLVCOMPAREEX pfnCompare, void *pContext)
{
    void *data = m_arrItems.GetData();
    qsort_s(m_arrItems.GetData(), m_arrItems.GetCount(), sizeof(SOUI::SItemPanel *), pfnCompare,
            pContext);
    m_iSelItem = -1;
    m_iHoverItem = -1;
    UpdatePanelsIndex();
