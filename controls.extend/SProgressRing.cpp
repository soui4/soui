...
SProgressRing::SProgressRing(void)
    : m_crBackground(RGBA(0xff, 0xff, 0xff, 0xff))
    , m_crForeground(RGBA(58, 171, 249, 255))
    , m_fArcRate(0.2f)
{
}
...
void SProgressRing::OnPaint(IRenderTarget *pRT)
{
    ...
    SAutoRefPtr<IPenS> pPenBack, pPenFore, oldPen;
    ...
    if (m_bShowPercent)
    {
        ...
        pRT->DrawText(strPercent, strPercent.GetLength(), rcClient,
