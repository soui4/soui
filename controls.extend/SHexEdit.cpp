...
class SHexEditAction {
  ...
  public:
    ...
    const SByteArray &getReplaceData() const
    {
        return replaceData;
    }
    UINT getReplaceLen() const
    {
        return replaceData.Size();
    }
    const SByteArray &getInsertData() const
    {
        return insertData;
    }
    UINT getInsertLen() const
    {
        return insertData.Size();
    }
    SHexEditAction *getNext() const
    {
        return next;
    }
};
...
// macros
#define NORMALIZE_SELECTION(beg, end) \
    if (beg > end)                    \
    {                                 \
        UINT tmp = end;               \
        end = beg;                    \
...
void SHexEdit::PaintAddresses(IRenderTarget *pRT)
{
    ...
    for (; nAdr < nEndAdr; nAdr += m_tPaintDetails.nBytesPerRow)
    {
        _sntprintf(pBuf, 32, (LPCTSTR)cAdrFormatString,
                   nAdr + m_dwZeroAddr); // slightly faster then CString::Format
...
void SHexEdit::SetEditCaretPos(UINT nOffset, bool bHighBits)
{
    ...
    CreateEditCaret(m_overwriteMode ? 2 : nCarretHeight - 1,
                    m_overwriteMode ? nCarretWidth : 1); // FIXME: hardcoded insert caret width 2
...
{
    if (!m_bHasCaret || (nCaretHeight != m_nCurCaretHeight) || (nCaretWidth != m_nCurCaretWidth))
    {
        m_bHasCaret = true;
...
void SHexEdit::OnEditPaste()
{
    ...
    int nHexDigits = countHexDigits(clipText.c_str(),
                                    clipText.length()); // < 0 or odd if non valid hex encoding
...
bool SHexEdit::OnEditInput(WORD nInput)
{
    ...
    else if (m_nCurrentAddress < GetDataSize())
    { // we are overwriting 1 char
        replaceLen = 1;
    }
    if (!m_bCaretAscii)
        if (m_bHighBits)
