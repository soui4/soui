...
{
    if (pRealWnd->GetRealWndParam().m_strClassName.CompareNoCase(
            CScintillaWnd::GetScintillaWndClass())
        == 0)
    {
        CScintillaWnd *pWnd = new CScintillaWnd();
        BOOL bOK = pWnd->Create(pRealWnd->GetRealWndParam().m_strWindowName, WS_CHILD,
                                CRect(0, 0, 0, 0), pRealWnd->GetContainer()->GetHostHwnd(),
                                pRealWnd->GetID(), SApplication::getSingleton().GetInstance());
        if (!bOK)
        {
            SASSERT(FALSE);
            delete pWnd;
...
BOOL SRealWndHandler_Scintilla::OnRealWndPosition(SRealWnd *pRealWnd, const CRect &rcWnd)
{
    ...
    {
        ::SetWindowPos(hRealWnd, 0, rcWnd.left, rcWnd.top, rcWnd.Width(), rcWnd.Height(),
                       SWP_NOZORDER);
