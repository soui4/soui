...
SPropertyItemColor::SPropertyItemColor(SPropertyGrid *pOwner)
    : SPropertyItemText(pOwner)
...
void SPropertyItemColor::DrawItem(IRenderTarget *pRT, CRect rc)
{
    ...
    else
    {
        ...
        for (int y = rcColor.top; y < rcColor.bottom; y += KTransGridSize)
        {
            ...
            {
                if (bDraw)
                    pRT->FillSolidRect(CRect(CPoint(x, y), CSize(KTransGridSize, KTransGridSize)), RGBA(0xcc, 0xcc, 0xcc, 0xff));
                bDraw = !bDraw;
            }
            bDrawY = !bDrawY;
        }
