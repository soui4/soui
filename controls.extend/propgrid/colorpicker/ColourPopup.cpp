...
struct ColourTableEntry
{
    ...
    { RGB(0x00, 0x00, 0x00), _T("黑色") },   { RGB(0xA5, 0x2A, 0x00), _T("棕色") },     { RGB(0x00, 0x40, 0x40), _T("橄榄绿色") }, { RGB(0x00, 0x55, 0x00), _T("暗绿色") }, { RGB(0x00, 0x00, 0x5E), _T("深青色") },     { RGB(0x00, 0x00, 0x8B), _T("深蓝色") }, { RGB(0x4B, 0x00, 0x82), _T("深紫蓝色") }, { RGB(0x28, 0x28, 0x28), _T("深灰色") },

    { RGB(0x8B, 0x00, 0x00), _T("深红色") }, { RGB(0xFF, 0x68, 0x20), _T("桔红色") },   { RGB(0x8B, 0x8B, 0x00), _T("深黄色") },   { RGB(0x00, 0x93, 0x00), _T("绿色") },   { RGB(0x38, 0x8E, 0x8E), _T("青色") },       { RGB(0x00, 0x00, 0xFF), _T("蓝色") },   { RGB(0x7B, 0x7B, 0xC0), _T("绿灰色") },   { RGB(0x66, 0x66, 0x66), _T("灰色") },

    { RGB(0xFF, 0x00, 0x00), _T("红色") },   { RGB(0xFF, 0xAD, 0x5B), _T("淡桔红色") }, { RGB(0x32, 0xCD, 0x32), _T("浅绿色") },   { RGB(0x3C, 0xB3, 0x71), _T("海蓝色") }, { RGB(0x7F, 0xFF, 0xD4), _T("浅绿色") },     { RGB(0x7D, 0x9E, 0xC0), _T("淡蓝色") }, { RGB(0x80, 0x00, 0x80), _T("紫罗兰色") }, { RGB(0x7F, 0x7F, 0x7F), _T("灰色") },

    { RGB(0xFF, 0xC0, 0xCB), _T("粉红色") }, { RGB(0xFF, 0xD7, 0x00), _T("金色") },     { RGB(0xFF, 0xFF, 0x00), _T("黄色") },     { RGB(0x00, 0xFF, 0x00), _T("亮绿色") }, { RGB(0x40, 0xE0, 0xD0), _T("绿松色") },     { RGB(0xC0, 0xFF, 0xFF), _T("天蓝色") }, { RGB(0x48, 0x00, 0x48), _T("梅红") },     { RGB(0xC0, 0xC0, 0xC0), _T("淡灰色") },

    { RGB(0xFF, 0xE4, 0xE1), _T("玫瑰色") }, { RGB(0xD2, 0xB4, 0x8C), _T("棕褐色") },   { RGB(0xFF, 0xFF, 0xE0), _T("淡黄色") },   { RGB(0x98, 0xFB, 0x98), _T("浅绿色") }, { RGB(0xAF, 0xEE, 0xEE), _T("浅绿宝石色") }, { RGB(0x68, 0x83, 0x8B), _T("浅蓝色") }, { RGB(0xE6, 0xE6, 0xFA), _T("淡紫色") },   { RGB(0xFF, 0xFF, 0xFF), _T("白色") },
};
...
BOOL CColourPopup::Create(CPoint p, COLORREF crColour, LPCTSTR szDefaultText /* = NULL */, LPCTSTR szCustomText /* = NULL */)
{
    m_crColour = m_crInitialColour = crColour;

    if (!CreateNative(NULL, WS_VISIBLE | WS_POPUP, WS_EX_TOOLWINDOW, p.x, p.y, 100, 100, // size updated soon
                      m_hOwner, 0, NULL))
        return FALSE;
...
BOOL CColourPopup::OnEraseBkgnd(HDC hdc)
{
    RECT rc;
...
void CColourPopup::SetWindowSize()
{
    ...
    m_WindowRect.SetRect(rect.left, rect.top, rect.left + m_nNumColumns * m_nBoxSize + 2 * m_nMargin, rect.top + m_nNumRows * m_nBoxSize + 2 * m_nMargin);
    ...
    if (m_strDefaultText.GetLength())
    {
        ...
        m_DefaultTextRect.SetRect(m_nMargin, m_nMargin, m_nMargin + TextSize.cx, 2 * m_nMargin + TextSize.cy);
    ...
    if (m_strCustomText.GetLength())
    {
        ...
        // Work out the text area
        m_CustomTextRect.SetRect(m_nMargin, m_WindowRect.Height(), m_nMargin + TextSize.cx, m_WindowRect.Height() + m_nMargin + TextSize.cy);
...
void CColourPopup::ChangeSelection(int nIndex)
{
    ...
    if ((m_nCurrentSel >= 0 && m_nCurrentSel < KNumColours) || m_nCurrentSel == CUSTOM_BOX_VALUE || m_nCurrentSel == DEFAULT_BOX_VALUE)
    {
...
void CColourPopup::DrawCell(HDC hdc, int nIndex)
{
    ...
    if (m_strCustomText.GetLength() && nIndex == CUSTOM_BOX_VALUE)
    {
        ...
        FillSolidRect(hdc, m_CustomTextRect.left + 2 * m_nMargin, m_CustomTextRect.top, m_CustomTextRect.Width() - 4 * m_nMargin, 1, ::GetSysColor(COLOR_3DSHADOW));
        FillSolidRect(hdc, m_CustomTextRect.left + 2 * m_nMargin, m_CustomTextRect.top + 1, m_CustomTextRect.Width() - 4 * m_nMargin, 1, ::GetSysColor(COLOR_3DHILIGHT));
    ...
    HBRUSH brush = CreateSolidBrush(RGB(GetRValue(GetColour(nIndex)), GetGValue(GetColour(nIndex)), GetBValue(GetColour(nIndex))));
    HPEN pen = CreatePen(PS_SOLID, 1, ::GetSysColor(COLOR_3DSHADOW));
