...
class SImagePlayer
    ...
    DEF_SOBJECT(SWindow, L"imgplayer") //定义GIF控件在XM加的标签
  public:
    SImagePlayer();
    ~SImagePlayer();
  ...
  protected
      : //消息处理，SOUI控件的消息处理和WTL，MFC很相似，采用相似的映射表，相同或者相似的消息映射宏
    /**
     * OnPaint
     * @brief    窗口绘制消息响应函数
     * @param    IRenderTarget * pRT --  绘制目标
     * @return   void
     * Describe  注意这里的参数是IRenderTarget
     * *,而不是WTL中使用的HDC，同时消息映射宏也变为MSG_WM_PAINT_EX
     */
