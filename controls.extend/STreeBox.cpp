...
static const wchar_t NAME_SWITCH[] =   L"switch";
    ...
    STreeItem::STreeItem(IHostProxy *pFrameHost,IItemContainer *pContainer)
    : SItemPanel(pFrameHost,pContainer)
    , m_bCollapsed(FALSE)
    , m_bVisible(TRUE)
    , m_nLevel(0)
{
...
HSTREEITEM STreeBox::InsertItem(SXmlNode xmlNode,DWORD dwData,HSTREEITEM hParent/*=STVI_ROOT*/, HSTREEITEM hInsertAfter/*=STVI_LAST*/,BOOL bEnsureVisible/*=FALSE*/)
{
...
void STreeBox::OnSize( UINT nType,CSize size )
{
    ...
    if(szView.cy>rcClient.Height())
        szView.cx = rcClient.Width()-GetSbWidth();
    else
...
void STreeBox::RedrawItem(HSTREEITEM hItem)
{
    ...
    if(rcItem.IsRectEmpty())
        return;
    rcItem.OffsetRect(-CPoint(m_ptOrigin));
...
BOOL STreeBox::FireEvent(IEvtArgs *evt)
{
...
void STreeBox::UpdateItemWidth(HSTREEITEM hItem,int nWidth)
{
    if(hItem != STVI_ROOT)
    {
        ...
        pItem->Move2(0,0,nWidth-xOffset,pItem->m_nItemHeight);
    }
...
BOOL STreeBox::OnItemStateChanged( IEvtArgs *pEvt )
{
