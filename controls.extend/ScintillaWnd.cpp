...
BOOL CScintillaWnd::Create(LPCTSTR lpszWindowName,
                           DWORD dwStyle,
                           const RECT &rect,
                           HWND hParent,
                           UINT nID,
                           HINSTANCE hInst)
{
    HWND hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, KScintillaClass, lpszWindowName, dwStyle,
...
void CScintillaWnd::UpdateLineNumberWidth(void)
{
    ...
    sprintf((tchLines), (" %i "), SendMessage(SCI_GETLINECOUNT, 0, 0));

    iLineMarginWidthNow = SendMessage(SCI_GETMARGINWIDTHN, 0, 0);
    iLineMarginWidthFit = SendMessage(SCI_TEXTWIDTH, STYLE_LINENUMBER, (LPARAM)tchLines);
...
BOOL CScintillaWnd::OpenFile(LPCTSTR lpFileName)
{
    ...
    SendMessage(SCI_ADDTEXT, str.GetLength(), reinterpret_cast<LPARAM>((LPCSTR)str));
