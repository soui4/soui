...
DEF_EVT_EXT(EventTBGetDispInfo,EVT_EXTERNAL_BEGIN+400,{
	HSTREEITEM  hItem;
	SWindow     *pItemWnd;
	BOOL        bSel;
	BOOL        bHover;
...
DEF_EVT_EXT(EventTBSelChanging,EVT_EXTERNAL_BEGIN+401,{
	HSTREEITEM hNewSel;
	HSTREEITEM hOldSel;
	BOOL        bCancel;
});

DEF_EVT_EXT(EventTBSelChanged,EVT_EXTERNAL_BEGIN+402,{
	HSTREEITEM hNewSel;
	HSTREEITEM hOldSel;
});

DEF_EVT_EXT(EventTBQueryItemHeight,EVT_EXTERNAL_BEGIN+403,{
	HSTREEITEM hItem;
...
public:
     ...
     * Describe  构造函数  
     */
    STreeItem(IHostProxy *pFrameHost,IItemContainer *pContainer);
...
{
    DEF_SOBJECT(SScrollView, L"treebox")
public:
    /**
     * STreeBox::STreeBox
     * @brief    构造函数
     ...
     * Describe  插入新项
     */
    HSTREEITEM InsertItem(SXmlNode xmlNode,DWORD dwData,HSTREEITEM hParent=STVI_ROOT, HSTREEITEM hInsertAfter=STVI_LAST,BOOL bEnsureVisible=FALSE);
