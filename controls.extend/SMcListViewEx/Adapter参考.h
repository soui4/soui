...
class SMusicListAdapter : public SMcAdapterBaseEx {
  ...
  public:
    SMusicListAdapter(int id, HWND nofitywnd)
        : m_id(id)
        , m_nofitywnd(nofitywnd)
        , m_curren_play(0)
    {
    ...
    virtual void getView(int position, SWindow *pItem, pugi::xml_node xmlTemplate)
    {
        ...
        pItem->GetEventSet()->subscribeEvent(
            EventSwndStateChanged::EventID,
            Subscriber(&SMusicListAdapter::OnItemStateChanged, this));
        pItem->FindChildByName(L"btn_play")
            ->GetEventSet()
            ->subscribeEvent(EVT_CMD, Subscriber(&SMusicListAdapter::btn_play, this));
    ...
    {
        for (xmlTemplate = xmlTemplate.first_child(); xmlTemplate;
