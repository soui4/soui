...
class SMCListViewDataSetObserverEx : public TObjRefImpl<ILvDataSetObserver> {
  public:
    SMCListViewDataSetObserverEx(SMCListViewEx *pView)
        : m_pOwner(pView)
    {
    }
...
int SMCListViewEx::InsertColumn(int nIndex, LPCTSTR pszText, int nWidth, UINT fmt, LPARAM lParam)
{
    SASSERT(m_pHeader);
...
BOOL SMCListViewEx::CreateChildren(SXmlNode xmlNode)
{
    ...
    m_pHeader = sobj_cast<SHeaderCtrlEx>(SApplication::getSingletonPtr()->CreateWindowByName(
        xmlHeader.attribute(L"wndclass").as_string(SHeaderCtrlEx::GetClassName())));
    SASSERT(m_pHeader);
    if (!m_pHeader)
        return FALSE;
    ...
    m_pHeader->GetEventSet()->subscribeEvent(
        EventHeaderItemChanging::EventID, Subscriber(&SMCListViewEx::OnHeaderSizeChanging, this));
    m_pHeader->GetEventSet()->subscribeEvent(EventHeaderItemSwap::EventID,
                                             Subscriber(&SMCListViewEx::OnHeaderSwap, this));
    m_pHeader->GetEventSet()->subscribeEvent(EventHeaderClick::EventID,
                                             Subscriber(&SMCListViewEx::OnHeaderClick, this));
...
void SMCListViewEx::UpdateHeaderCtrl(bool _bNewSize)
{
    ...
    if (m_pHeader)
        m_pHeader->Move(rcHeader, _bNewSize);
}
...
BOOL SMCListViewEx::OnHeaderClick(IEvtArgs *pEvt)
{
    ...
    SASSERT(pEvt2);
    SHDITEM hi;
    hi.mask = SHDI_ORDER | SHDI_FORMAT;
    ...
    if (m_adapter->OnSort(iCol, pFmts, m_pHeader->GetItemCount()))
    {
        //更新表头的排序状态
        for (int i = 0; i < m_pHeader->GetItemCount(); i++)
        {
...
void SMCListViewEx::UpdateVisibleItems()
{
    ...
    if (iNewFirstVisible != -1)
    {
        while (pos < m_siVer.nPos + (int)m_siVer.nPage && iNewLastVisible < m_adapter->getCount())
        {
            ...
            if (m_adapter->IsSel(iNewLastVisible))
            ...
            { // create new visible item
                ...
                { //创建一个新的列表项
                    ...
                    ii.pItem->GetEventSet()->subscribeEvent(
                        EventItemPanelClick::EventID,
                        Subscriber(&SMCListViewEx::OnItemClick, this));
                }
                else
                {
    ...
    else if (uMsg == WM_LBUTTONUP || uMsg == WM_RBUTTONUP || uMsg == WM_MBUTTONUP)
    {
        if (uMsg == WM_LBUTTONUP)
        {
            if (m_bSwap)
            {
                ...
                m_adapter->SwapEnd(
                    pHover ? (m_bSwapToTop ? pHover->GetItemIndex() : pHover->GetItemIndex() + 1)
                           : -1);
                m_bSwap = false;
            }
            if (m_bDrop)
