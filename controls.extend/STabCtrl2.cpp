...
void STabCtrl2::OnPaint(IRenderTarget *pRT)
{
    ...
    for (size_t i = 0; i < GetItemCount(); i++)
    {
        ...
        if (i > 0 && m_pSkinTabInter
            && ((i < (m_nCurrentPage)) || (i > (m_nCurrentPage + 1)) /*选中页左右不用画分隔线*/))
        {
            rcSplit = rcItem;
...
int STabCtrl2::OnCreate(void *)
{
    ...
    GetEventSet()->subscribeEvent(EventTabItemLeave::EventID,
                                  Subscriber(&STabCtrl2::OnItemLeave, this));
    return iRet;
}
...
{
    if (wcscmp(xmlPage.name(), STabPage2::GetClassName()) != 0)
        return NULL;
