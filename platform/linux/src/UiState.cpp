...
SUiState::~SUiState()
{
    ...
    assert(m_thdStates.IsEmpty());
    SPOSITION pos = m_thdStates.GetStartPosition();
    while(pos){
        SThreadUiState *pstate = m_thdStates.GetNextValue(pos);
        delete pstate;
    }
    m_thdStates.RemoveAll();
}
...
bool SUiState::init(int screenNum)
{
    ...
    if(m_thdStates.Lookup(tid) != NULL){
        return true;
    }
...
void SUiState::uninit()
{
    SAutoWriteLock autoLock(&m_rwLock);
    pthread_t tid = pthread_self();
    auto it = m_thdStates.Lookup(tid);
    if(it == NULL){
        return;
    }
    delete it->m_value;
    m_thdStates.RemoveAtPos(it);
}
