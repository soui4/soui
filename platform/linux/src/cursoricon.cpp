...
HCURSOR WINAPI CreateCursor( HINSTANCE instance, int hotspot_x, int hotspot_y,
                             int width, int height, const void *and_, const void *xor_)
{
    ICONINFO info;
    HCURSOR cursor;
...
HICON WINAPI CreateIcon( HINSTANCE instance, int width, int height, BYTE planes,
                         BYTE depth, const void *and_, const void *xor_)
{
    ICONINFO info;
    HICON icon;
...
{
    return (HCURSOR)LoadImageW( hInstance, name, IMAGE_CURSOR, 0, 0,
...
HICON WINAPI LoadIconW(HINSTANCE hInstance, LPCWSTR name)
{
    ...
    return (HICON)LoadImageW( hInstance, name, IMAGE_ICON, 0, 0,
                       LR_SHARED | LR_DEFAULTSIZE );
}
                              ...
                              HBITMAP src, int width, int height )
{
    ...
    if (!SelectObject( hdc, src ))  /* do it the hard way */
    {
        ...
        if (!(info = (BITMAPINFO *)HeapAlloc( GetProcessHeap(), 0, FIELD_OFFSET( BITMAPINFO, bmiColors[256] )))) return;
        info->bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
        info->bmiHeader.biWidth = width;
                            ...
                            INT desiredx, INT desiredy, UINT loadflags )
{
    ...
    else
    {
        ...
        if (info->bmiHeader.biHeight > 65535 || info->bmiHeader.biWidth > 65535) {
            WARN_(curor)("Broken BitmapInfoHeader!\n");
            goto end;
        }
