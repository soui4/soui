...
BOOL WINAPI SetRectRgn(HRGN hrgn, INT left, INT top, INT right, INT bottom)
{
    ...
    if (!(obj = (WINEREGION * )GDI_GetObjPtr(hrgn, NTGDI_OBJ_REGION))) return FALSE;
...
BOOL REGION_FrameRgn(HRGN hDest, HRGN hSrc, INT x, INT y)
{
    ...
    {
        if (!(destObj = (WINEREGION*)GDI_GetObjPtr(hDest, NTGDI_OBJ_REGION))) goto done;
...
INT mirror_region(HRGN dst, HRGN src, INT width)
{
    ...
    if (!(src_rgn = (WINEREGION * )GDI_GetObjPtr(src, NTGDI_OBJ_REGION))) return ERROR;
