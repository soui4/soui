...
{
	std::vector<T>::iterator it = arr.begin();
	while(it!=arr.end()){
		if(*it == ele){
			arr.erase(it);
			return TRUE;
...
STDMETHODIMP_(BOOL) SMessageLoop::AddMessageFilter(THIS_ IMsgFilter *pMessageFilter)
{
	return m_priv->AddMessageFilter(pMessageFilter);
}

STDMETHODIMP_(BOOL) SMessageLoop::RemoveMessageFilter(THIS_ IMsgFilter *pMessageFilter)
{
	return m_priv->RemoveMessageFilter(pMessageFilter);
}

STDMETHODIMP_(BOOL) SMessageLoop::AddIdleHandler(THIS_ IIdleHandler *pIdleHandler)
{
	return m_priv->AddIdleHandler(pIdleHandler);
}

STDMETHODIMP_(BOOL) SMessageLoop::RemoveIdleHandler(THIS_ IIdleHandler *pIdleHandler)
{
	return m_priv->RemoveIdleHandler(pIdleHandler);
}

STDMETHODIMP_(BOOL) SMessageLoop::PreTranslateMessage(THIS_ MSG *pMsg)
{
	return m_priv->PreTranslateMessage(pMsg);
}

STDMETHODIMP_(BOOL) SMessageLoop::OnIdle(THIS_ int nIdleCount)
{
	return m_priv->OnIdle(nIdleCount);
}

STDMETHODIMP_(int) SMessageLoop::Run(THIS)
{
	return m_priv->Run();
}

STDMETHODIMP_(BOOL) SMessageLoop::IsRunning(THIS) const
{ 
	return m_priv->IsRunning();
}

STDMETHODIMP_(void) SMessageLoop::OnMsg(THIS_ LPMSG pMsg)
{
	m_priv->OnMsg(pMsg);
}

STDMETHODIMP_(void) SMessageLoop::Quit(THIS_ int exitCode)
{
	m_priv->Quit(exitCode);
}

STDMETHODIMP_(BOOL) SMessageLoop::PostTask(THIS_ IRunnable *runable)
{
	return m_priv->PostTask(runable);
}

STDMETHODIMP_(int) SMessageLoop::RemoveTasksForObject(THIS_ void *pObj)
{
	return m_priv->RemoveTasksForObject(pObj);
}

STDMETHODIMP_(void) SMessageLoop::ExecutePendingTask()
{
	m_priv->ExecutePendingTask();
}

STDMETHODIMP_(BOOL) SMessageLoop::PeekMsg(THIS_ LPMSG pMsg, UINT wMsgFilterMin, UINT wMsgFilterMax, BOOL bRemove)
{
	return m_priv->PeekMsg(pMsg,wMsgFilterMin,wMsgFilterMax,bRemove);
}

STDMETHODIMP_(BOOL) SMessageLoop::WaitMsg(THIS)
{
	return m_priv->WaitMsg();
}

STDMETHODIMP_(int) SMessageLoop::HandleMsg(THIS)
{
