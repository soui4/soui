...
7. 原代码中使用的IPen, IBrush, IBitmap, IRegion, IPath等IRenderObj对象统一增加S后缀，变为IPenS, IBrushS, IBitmapS, IRegionS, IPathS等，主要是C接口中不支持命名空间，导致可能和系统接口重名。
8. 为了兼容C接口，所有接口的方法都没有了默认参数。比如IRenderTarget.SelectObject，3.0第二个参数默认为NULL, 新版本没有默认参数，必须写全参数。
