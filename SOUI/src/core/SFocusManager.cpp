...
SWindow *FocusSearch::FindNextFocusableView(SWindow *starting_view, bool reverse, bool check_starting_view)
{
    ...
    if (!starting_view)
    {
        if (reverse)
        {
            ...
            while (starting_view->GetChildrenCount())
                starting_view = starting_view->GetWindow(GSW_LASTCHILD);
        }
        else
        {
...
SWindow *FocusSearch::FindNextFocusableViewImpl(SWindow *starting_view, bool check_starting_view, bool can_go_up, bool can_go_down, SWindow *pSkipGroupOwner)
{
    ...
    if (can_go_down)
    {
        ...
        if (pChild)
        {
            SWindow *v = FindNextFocusableViewImpl(pChild, true, false, true, pSkipGroupOwner);
            if (v)
            {
                return v;
            }
    ...
    if (sibling)
    {
        SWindow *v = FindNextFocusableViewImpl(sibling, true, false, true, pSkipGroupOwner);
        if (v)
        {
            return v;
        }
    ...
    if (can_go_up)
    {
        ...
        while (parent)
        {
            ...
            if (sibling)
            {
                return FindNextFocusableViewImpl(sibling, true, true, true, pSkipGroupOwner);
            }
...
SWindow *FocusSearch::FindPreviousFocusableViewImpl(SWindow *starting_view, bool check_starting_view, bool can_go_up, bool can_go_down, SWindow *pSkipGroupOwner)
{
    ...
    if (can_go_up)
    {
        ...
        if (pParent)
            return FindPreviousFocusableViewImpl(pParent, true, true, false, pSkipGroupOwner);
    }
...
bool FocusSearch::IsViewFocusableCandidate(SWindow *v, SWindow *pGroupOwner)
{
    if (!IsFocusable(v))
        return false;
    if (pGroupOwner && v->IsSiblingsAutoGroupped() && v->GetParent() == pGroupOwner)
        return false;
    return true;
}
...
BOOL SFocusManager::IsTabTraversalKey(UINT vKey)
{
    ...
    if (pFocus && pFocus->OnGetDlgCode() & SC_WANTTAB)
        return FALSE;
