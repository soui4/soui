...
void STipCtrl::ShowTip(BOOL bShow)
{
    ...
    else if (!m_strTip.IsEmpty())
    {
        ...
        if(m_tipAlign == TA_AUTO){
            ...
            case TA_X_LEFT:pt.x = m_rcTarget.left - rcWnd.Width(); break;
            case TA_X_RIGHT:pt.x = m_rcTarget.right; break;
            case TA_X_CENTER:pt.x -= rcWnd.Width() / 2; break;
            }
            switch (m_tipAlign & TA_Y_MASK) {
            case TA_Y_TOP:pt.y = m_rcTarget.top - rcWnd.Height(); break;
            case TA_Y_BOTTOM:pt.y = m_rcTarget.bottom; break;
            case TA_Y_CENTER:pt.y -= rcWnd.Height() / 2; break;
            }
            rcWnd.MoveToXY(pt);
        }
