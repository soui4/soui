...
# 格式: {文件名: (子图数量, 是否水平排列)}
IMAGE_CONFIG_ALPHA = {
    'ready.bmp': (2, True),      # 2个水平排列的子图
    '出牌.bmp': (5, True),      # 5个水平排列的子图
    '庄家.bmp': (2, True),       # 2个水平排列的子图
    '开始.bmp': (5, True),      # 5个水平排列的子图
    '恢复.bmp': (4, True),       # 4个水平排列的子图
    '扣底牌.bmp': (5, True),    # 5个水平排列的子图
    '最大化.bmp': (4, True),     # 4个水平排列的子图
    '最小化.bmp': (4, True),     # 4个水平排列的子图
    '退出.bmp': (4, True),       # 4个水平排列的子图
    '闹钟.bmp': (2, True),       # 2个水平排列的子图
    '花色.bmp': (2, True),       # 4个水平排列的子图
    'levella.bmp': (2, True),       # 4个水平排列的子图
    'levela.bmp': (2, True),       # 4个水平排列的子图
}

# 定义使用mask模式的图像配置信息
# 格式: {文件名: (子图数量, 是否水平排列)}
IMAGE_CONFIG_MASK = {
    '上轮牌按纽.bmp': (5, True),  # 1个水平排列的子图
    '亮主-放弃.bmp': (5, True),  # 1个水平排列的子图
    '亮主-方块.bmp': (5, True),  # 1个水平排列的子图 
     '看底牌按纽.bmp': (5, True),  # 1个水平排列的子图
}
...
def process_directory(input_dir, output_dir):
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith('.bmp'):
            ...
            try:
                # 首先检查是否在alpha配置中
                if filename in IMAGE_CONFIG_ALPHA:
                    subimage_info = IMAGE_CONFIG_ALPHA.get(filename)
                    convert_bmp_to_png_with_alpha(input_path, output_path, subimage_info, use_mask=False)
                # 然后检查是否在mask配置中
                elif filename in IMAGE_CONFIG_MASK:
                    ...
                    # 不在任何配置中，使用自动检测
                    print(f"警告: {filename} 不在配置中，使用自动检测")
                    convert_bmp_to_png_with_alpha(input_path, output_path)
            except Exception as e:
                print(f"处理 {input_path} 时出错: {e}")

def main():
    # 定义输入和输出目录
    input_dir = 'uires/image'
    output_dir = 'uires/image_png'
