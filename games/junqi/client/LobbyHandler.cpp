...
BOOL LobbyHandler::OnLoginAck(const void *lpData, int nSize)
{
    ...
    SStringT strInfo;
    if(pAck->dwProps[PROPID_BOMBFLY])
    {
        ss<< L"炸弹飞,";
    }
    if(pAck->dwProps[PROPID_MINEASBOMB])
    {
        ss<< L"炸弹雷,";
    }
    ss<< L"和棋步数:"<<pAck->dwProps[PROPID_PEACESTEPS];
    switch(pAck->dwProps[PROPID_RULE])
    {
    case RULE_NOR:
        ss<< L"四暗,";
        break;
    ...
    ss<< L"步时:"<<pAck->dwProps[PROPID_STEPTICK];
    ss<< L"布阵时间:"<<pAck->dwProps[PROPID_LAYOUTTICK];
    ss<< L"待机次数:"<<pAck->dwProps[PROPID_PASS];
    ss<< L"自动存盘:"<<pAck->dwProps[PROPID_AUTOSAVE];
    ss<< L"拿子闪烁:"<<pAck->dwProps[PROPID_GETFLASH];

    strInfo = S_CW2T(ss.str().c_str());
