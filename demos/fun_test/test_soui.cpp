...
// swinx 扩展全屏命令（swinx/include/winuser.h 中定义）；Windows 编译使用系统
// SDK 的 windows.h，没有该定义，这里兜底。仅 macOS 后端实现了该命令。
#ifndef SC_FULLSCREEN
...
public:
    ...
    void OnInit(IEvtArgs* e) {
        ...
        // 全屏命令（SC_FULLSCREEN）目前仅 macOS 后端实现，restore 与之配套
        // 用于退出原生全屏，非 Apple 平台一并隐藏入口
        {
            BOOL bShow = FALSE;
#ifdef __APPLE__
            bShow = TRUE;
#endif
            SWindow* btnFs = FindChildByName("btn_fullscreen");
            if (btnFs)
                btnFs->SetVisible(bShow, TRUE);
            SWindow* btnRestore = FindChildByName("btn_restore");
            if (btnRestore)
                btnRestore->SetVisible(bShow, TRUE);
        }
    }
    ...
    void OnBtnFullscreen() {
        // 发送 swinx 扩展全屏命令：macOS 上进入原生全屏（独占新桌面/Space），
        // 再次发送退出全屏；其它平台为无操作（按钮也已在非 Apple 平台隐藏）。
        PostMessage(WM_SYSCOMMAND, SC_FULLSCREEN, 0);
    }

    void OnBtnRestore() {
        // 退出全屏/还原窗口：macOS 端 sendNsSysCommand 对 SC_RESTORE 先退出
        // 原生全屏，再 deminiaturize/unzoom。
