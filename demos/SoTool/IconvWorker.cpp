...
static const BYTE g_byUtf8BOM[] = {0xEF, 0xBB, 0xBF};
static const BYTE g_byUnicodeBOM[] = {0xFF, 0xFE};
...
std::wstring CIconvWorker::GetDstPath(LPCWSTR szSrcPath)
{
    if(m_bOverwrite)
        return szSrcPath;
    SStringW str(szSrcPath);
    int pos = str.ReverseFind(PATH_SLASH);
    if (pos != -1)
    {
        str = str.Mid(pos + 1);    
    }
    return m_strTargetPath + str.c_str();   
}
