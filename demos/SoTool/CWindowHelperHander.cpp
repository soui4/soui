...
void CWindowHelperHander::GetStyleList(LONG lStyle, SArray<StyleInf>& styleList)
{
	...
	if (lStyle)
	{
		styleList.Add(StyleInf(SStringT().Format(_T("0x%08x"),lStyle),_T("δ֪��ʽ")));
	}
}
...
void CWindowHelperHander::GetExStyleList(LONG lStyle, SArray<StyleInf>& styleList)
{
	...
	if (lStyle)
	{
		styleList.Add(StyleInf(SStringT().Format(_T("0x%08x"), lStyle), _T("δ֪��չ��ʽ")));
	}
}
...
{	
	...
	if (pStyleList)
	{
		...
		for (int i = 0; i < styleList.GetCount(); i++)
		{
			...
			SXmlNode nextNode=pChildNode.next_sibling();
			pChild2->InitFromXml(&nextNode);
		}
	}
