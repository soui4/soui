...
void CMainDlg::ContactTVItemClick(int nGID, const std::string& strID)
{
    ...
    {
        //群聊
        auto iter = CGlobalUnits::instance()->m_mapGroups.find(strID);
        if (iter != CGlobalUnits::instance()->m_mapGroups.end())
        {
            std::string strName = iter->second.m_strGroupName;
            pTitle->SetWindowTextW(S_CA2T(strName.c_str()));
    ...
    {
        //好友
        auto iter = CGlobalUnits::instance()->m_mapPersonals.find(strID);
